...
function PathwayCourse() {
  ...
  const { myInterval, setMyInterval } = useContext(MyContexts);
  ...
  const API_URL = 'https://merd-api.merakilearn.org/merakihackthon/28Sep/merakiTrack'

  const postData = (email, token) => {
    fetch(API_URL, {
      method: 'POST',
      headers: {
  ...
  useEffect(() => {
    ...
    if (user?.data?.token) {
        ...
        postData(user.data.user.email, user.data.token);
      }, 1000))
    }
  }, []);
